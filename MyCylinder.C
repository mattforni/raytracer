...
intersectRtn MyCylinder::intersect(intersectRtn currObj)
{
	...
	double a= (d_tilde[0]*d_tilde[0]+d_tilde[2]*d_tilde[2]);
	double b= (2.0*p_tilde[0]*d_tilde[0]+2.0*p_tilde[2]*d_tilde[2]);
	double c= (p_tilde[0]*p_tilde[0]+p_tilde[2]*p_tilde[2]-0.25);
...
// 		printf("t1 is equal to: %lf\n", t1);
// 		printf("t2 is equal to: %lf\n", t2);
...
// 		printf("the y value of ppv1: %lf\n", ppv1[1]);
// 		printf("the y value of ppv2: %lf\n", ppv2[1]);
		if (ppv1[1]<-0.5 || ppv1[1]>0.5)
			t1= INFINITY;
		if (ppv2[1]<-0.5 || ppv2[1]>0.5)
			t2= INFINITY;
// 		printf("after checks t1 is equal to: %lf\n", t1);
// 		printf("after checks t2 is equal to: %lf\n", t2);
		// if t2 <= t1 & >= 0 then set t1 equal to t2
		if (t2 <= t1 && t2 > 0)
			t1= t2;
// 		printf("now t1 is: %lf\n", t1);
	}
	// at this point t1 is holding the smallest value

	// checking the top
	t2= (0.5-p_tilde[1])/d_tilde[1];
	ppv2= p_tilde+t2*d_tilde;
	// checking to see if x^2 + z^2 > 0.25
	if (ppv2[0]*ppv2[0]+ppv2[2]*ppv2[2] > 0.25-EPSILON)
		t2= INFINITY;
	...
	currObj.oPoint.homogenize();
 	...
 	if (currObj.oPoint[1] <= 0.5+EPSILON &&  currObj.oPoint[1] >= 0.5-EPSILON) {
		...
		IAVector norm(currObj.oPoint[0], 0, currObj.oPoint[2]);
		currObj.oNorm = norm;
	}
...
CS123Color MyCylinder::map(IAPoint p, double u, double v, CS123Image* textMap)
{
	CS123Color rtnColor;
	...
	else if (y > -0.5-EPSILON && y < -0.5+EPSILON)
	{
		a = x + 0.5;
		b = 1.0 - (z + 0.5);
	}
